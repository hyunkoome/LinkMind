...
- **하이브리드 RAG (2026-06-03)**: 답변에 **위키 본문**(`linkmind_wiki_pages`) + item chunk + pinned 를 함께 활용 — 정리해둔 위키를 실제 답변에 써먹음(원본 조각만 보는 게 아니라).
- **대화 프라이버시**: 세션/메시지는 **소유자 본인만**(admin 도 남의 대화 X), 프로젝트는 조직 공유. 계정별 localStorage + 서버 동기화(`PUT /sessions/sync`, `GET /sessions/export` 복원).
- 다음: **arxiv 외부검색(agentic)** — 로컬 Gemma + 무료 arxiv API(외부 AI 없음); 이후 검색/QA/agentic 요청유형 구분.

</details>

<details>
<summary><b>🔐 멀티테넌트 / 인증</b></summary>

<br>

- **1조직 = 1space, 데이터 공유**; self-signup 없음(루트 관리자가 멤버 발급, 또는 bootstrap). space = 격리·학습·책임의 단위.
- **인증**: id/pw 로그인 + JWT httpOnly 쿠키 + 전 데이터 API 보호. **bootstrap**: user 0명이면 브라우저 `/login` 에 '조직 만들기'가 떠 첫 관리자 + 조직 생성.
- **멤버 발급**: 루트 관리자가 Settings(루트전용 `require_space_admin`)에서 이메일 + 초기 비번으로 발급 → 멤버는 첫 로그인 시 자격 강제 변경(force-change).
- **대화 프라이버시**: 세션은 소유자 본인만 조회, 학습은 space 전체(보기 권한 ≠ 학습 사용).
- **배포 모델**: 조직 서버 1대(backend + GPU + DB) + 가벼운 **웹/데스크탑(Tauri)** 클라이언트 — 유저 PC 는 GPU 불필요. per-row RLS 는 의도적으로 폐기(1조직 = 1인스턴스 = 격리 경계).
...
| **4** | 🚧 진행 | **멀티테넌트(조직 space/멤버발급/force-change/권한/대화프라이버시) ✅**, 대화형 `/ask` 멀티턴 ✅ + **하이브리드 RAG(위키 본문) ✅**, **논문 writer 재설계 ✅**(Docling raw→논문구조+inline 그림·표+한글 캡션) + **중복 위키 정리(T4)·예방·de-clone ✅** + 위키 렌더(GFM 표/LaTeX), **키워드 기반 arxiv 수집→위키(admin, MVP) ✅**(로컬 arxiv 메타 DB **3M편 FTS**·rate limit 0·**OAI 증분**(watermark 겹침+버전 upsert) + 3패널 UI + 위키 유/무 필터 + arXiv/위키/수집 3-state 버튼), **writer 큰논문 토큰 안정화 ✅**(섹션 map 압축 + output 동적 클램프), **classifier 연관위키 재합성 방지 ✅**; **(다음) writer 합성 검증 + Docling VRAM 경합 fix**, 실제 채널 확장(Slack/WhatsApp/Discord), 자가학습(**암묵 feedback** — 답변에 대한 다음 턴 뉘앙스·행동을 신호로 + 명시 교정; 👍/👎 버튼 아님), critic agent |
...
> 다음 우선순위: **키워드 기반 arxiv 수집(admin, MVP) ✅** — 로컬 arxiv 메타 DB(`arxiv_papers` FTS, Kaggle+OAI 적재)로 rate limit 0, 대표 키워드 그룹 검색 → 수집 → 위키. **(다음)** writer 큰논문 합성 검증(짤림 없는 고품질) + Docling VRAM 경합 fix(수집 PDF 가 item 으로 안 생기는 케이스) → 주기 자동수집 → **자가학습(feedback)** → **학습 파이프라인(Phase 5)**. 학습 파이프라인의 *인프라*(dataset exporter, QLoRA 셋업, 데이터셋 품질 검증)는 미리 만들 수 있음 — 자가학습과 병행; 실제 학습은 데이터가 충분히 쌓인 뒤.
