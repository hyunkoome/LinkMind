...
> LLM provider 는 OpenAI / Anthropic / Ollama 도 추상화로 지원. 모델·구동 설정은 DB(`app_settings`) + Settings UI 에서 관리하고 `scripts/vllm_restart.sh` 로 재구동한다.
...
<details>
<summary><b>step3~4 — LLM 모델 + Qdrant 컬렉션 + vLLM</b></summary>

```bash
# step3: Ollama 모델 (provider abstraction fallback)
...
# vLLM (Gemma 4 26B-A4B-AWQ) + vLLM-embed (bge-m3) 컨테이너
